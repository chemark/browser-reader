...
| Page | Mode | Selector |
|------|------|----------|
| WeChat Official Account | single read | `#js_content` |
| X/Twitter thread (`/status/`) | **scroll & collect** | author's tweets, top to bottom |
| X/Twitter other | single read | `[data-testid=articleContent]` |
| Everything else | single read | `article` → `main` → `body` |
...
| 页面类型 | 模式 | 选择器 |
|----------|------|--------|
| 微信公众号 | 单次读取 | `#js_content` |
| X/Twitter 推文串（`/status/`） | **滚动收集** | 从顶到底，按过滤模式收集 |
| X/Twitter 其他 | 单次读取 | `[data-testid=articleContent]` |
| 其他所有页面 | 单次读取 | `article` → `main` → `body` |
...
| ページ | モード | セレクター |
|--------|--------|-----------|
| WeChat公式アカウント | 単回読取 | `#js_content` |
| X/Twitterスレッド（`/status/`） | **スクロール収集** | 上から下へ、フィルターモードで収集 |
| X/Twitterその他 | 単回読取 | `[data-testid=articleContent]` |
| その他すべてのページ | 単回読取 | `article` → `main` → `body` |
...
| 페이지 | 모드 | 선택자 |
|--------|------|--------|
| WeChat 공식 계정 | 단순 읽기 | `#js_content` |
| X/Twitter 스레드 (`/status/`) | **스크롤 수집** | 위에서 아래로, 필터 모드로 수집 |
| X/Twitter 기타 | 단순 읽기 | `[data-testid=articleContent]` |
| 그 외 모든 페이지 | 단순 읽기 | `article` → `main` → `body` |
